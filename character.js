...
let urlCharacterPage = `https://rickandmortyapi.com/api/character/${id}`;

const paintSingleCharacter = (data) => {

    const characterSingleContainer = document.querySelector('#characterSingleContainer')
    characterSingleContainer.className = 'characterSingleContainer'

    const characterSingleCard = document.createElement('div')
    characterSingleCard.className = 'card'
    characterSingleCard.innerHTML = `
    <div id="nameStatusDiv" class="nameStatusDiv">
                <h3 id="name" class="name">${data.name}</h3>
                <h3 id="status" class="status">${data.status}</h3>
            </div>
            <div class=imageContainer>
                <img id="image" class="image" src="${data.image}" alt="${data.name}">
            </div>
            <div id=speciesGenderDiv class=speciesGenderDiv>
                <h4 id="species" class="species">${data.species}</h4>
                <h4 id="gender" class="gender">${data.gender}</h4>
            </div>
            <h3 id="location" class="location">${data.location.name}</h3>
            <h3 id="origin" class="origin">${data.origin.name}</h3>
            <h3 id="created" class="created">${data.created}</h3>

    `
    characterSingleContainer.append(characterSingleCard)
}
...
paintPageCharacter = (dataSingleCharacter) => {
    fetch(urlCharacterPage)
        .then((response) => response.json())
        .then((urlCharacterPageData) => {
            console.log(urlCharacterPageData)
            paintSingleCharacter(urlCharacterPageData)
